...
class BaseMultilayerPerceptron(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.importw = importw
        self.coef_initw=coef_initw
        self.intercept_initw=intercept_initw
    ...
    def _initialize(self, y, layer_units, dtype):
        ...
        elif self.importw == True:
            self.coefs_=self.coef_initw
            self.intercepts_=self.intercept_initw
    ...
    def _fit_stochastic(
        ...
        try:
            for it in range(self.max_iter):
                ...
                if self.verbose:
                    self.outputs.append("Iterasyon %d, loss = %.8f" % (self.n_iter_, self.loss_))
                    print("Iterasyon %d, loss = %.8f" % (self.n_iter_, self.loss_))
                ...
                if self._no_improvement_count > self.n_iter_no_change:
                    ...
                    if early_stopping:
                        msg = (
                            "Valid Skor ve Loss skor tolerans=%f ile %d iterasyondan beri gelişim gösteremedi."
                            % (self.tol, self.n_iter_no_change)
                        )
                        
                    else:
                        msg = (
                            "Eğitim Loss skor tolerans=%f ile %d iterasyondan beri gelişim gösteremedi."
                            % (self.tol, self.n_iter_no_change)
                        )
                ...
                if self.n_iter_ == self.max_iter:
                    self.outputs.append('Uyarı!! Koşullar sağlanamadan Maximum iterasyona ulaşıldı (%d) '% self.max_iter)
                    warnings.warn(
            ...
            self.outputs.append('Uyarı!! Eğitim kullanıcı tarafından durduruldu.')
        if early_stopping:
    ...
    def _update_no_improvement_count(self, early_stopping, X_val, y_val):
        if early_stopping:
            ...
            if self.verbose:
                ...
                self.outputs.append("Validation score: %f" % self.validation_scores_[-1])
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            max_fun=max_fun,
            importw=importw,
            coef_initw=coef_initw,
            intercept_initw=intercept_initw,
            outputs=outputs,
        )
